-- ==================================================
-- YOKUDO HUB | FEATURE | AFK System 2
...
local function JumpOutTreadmill(TreadmillPos, Callback)
    ...
    task.spawn(function()
        ...
        while Attempts < JUMP_MAX_ATTEMPTS do
            ...
            print("[AFK2] Jump Attempt " .. Attempts .. " | Dist: " .. DistToTreadmill)

            if DistToTreadmill > JUMP_DISTANCE_THRESHOLD then
                print("[AFK2] ✅ Jumped out! Distance:", DistToTreadmill)
                JumpedOut = true
                if Callback then Callback() end
                return
            end
        ...
        if not JumpedOut then
            print("[AFK2] ⚠️ JumpOut timeout after " .. Attempts .. " attempts")
        end
        if Callback then Callback() end
    end)
end
...
local function StartDistanceCheck()
    ...
    DistCheckThread = task.spawn(function()
        while AFKEnabled do
            ...
            print("[AFK2] Distance Check | Dist:", DistToTreadmill)

            if DistToTreadmill > DIST_TREADMILL_THRESHOLD then
                print("[AFK2] Player jumped out! Fly back to Treadmill")
...
local function EnableAFK()
    ...
    if MyTreadmill then
        ...
        print("[AFK2] Treadmill found:", MyTreadmill:GetFullName())
    else
        warn("[AFK2] Treadmill not found!")
        AFKEnabled = false
        return
    end

    -- ✅ Fly TP ទៅ Safe Zone មុន → រង់ចាំ → Fly TP ទៅ Treadmill
    print("[AFK2] Fly to Safe Zone first")
    FlyTP(SAFE_ZONE, function()
        task.wait(SAFE_WAIT_TIME)
        print("[AFK2] Safe Zone Reached → Fly to Treadmill")
        FlyTP(MyTreadmillPos, function()
            print("[AFK2] Arrived at Treadmill → Start Distance Check")
            StartDistanceCheck()
        end)
    end)

    print("[AFK2] AFK System 2: ON")
end
...
local function JumpOutAndGoSafe()
    ...
    JumpOutTreadmill(MyTreadmillPos, function()
        print("[AFK2] Jumped out, ready for attack")
    end)
end
...
print("✅ AFKSystem2 Feature Loaded (Safe Zone First + Jump Out)")
